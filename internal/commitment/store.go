...
type Entry struct {
	...
	LastReminderRefID string    `json:"last_reminder_ref_id,omitempty"`
	LastReminderMode  string    `json:"last_reminder_mode,omitempty"`
...
type Summary struct {
	...
	WithReminderHistory int            `json:"with_reminder_history"`
...
func (s *Store) Summary() (Summary, error) {
	...
	for _, item := range items {
		...
		if !item.LastReminderAt.IsZero() {
			out.WithReminderHistory++
		}
...
func (s *Store) NoteReminderIssued(id string, refID string, mode string, at time.Time) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	items, err := s.loadLocked()
	if err != nil {
	...
	for i := range items {
		if items[i].ID == id {
			items[i].LastReminderRefID = refID
			items[i].LastReminderMode = mode
			items[i].LastReminderAt = at
			items[i].ReminderCount++
			items[i].UpdatedAt = at
			return s.saveLocked(items)
		}
	}
