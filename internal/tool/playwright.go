...
func (t *BrowserWorkflowTool) Definition() types.ToolDefinition {
	return types.ToolDefinition{
		...
		Description: "Run a structured browser workflow. Prefer this over custom scripts for normal browsing. Use observe first or after navigation to see the current URL, page text, clickable links/buttons, inputs, indexes, and selectors. Later actions may target an observed element by index, selector, text, label, or placeholder. Common actions: observe, goto, click, type, press, scroll, wait_for, fill_form, login_form, extract_text, extract_table, screenshot, downloads, tabs, login checks, and pagination. press may target an element or the whole page; scroll supports direction=up/down/top/bottom. extract_text defaults to body. wait_for may be used without selector/text/url_contains as a plain delay/page-settle wait. Omit browser/headless/timeouts unless needed; defaults are filled automatically. With a persistent profile, Qorvexus keeps a live browser session and calls without start_url resume the last non-blank URL. Navigation waits default to domcontentloaded and individual actions default to short timeouts to avoid hangs.",
